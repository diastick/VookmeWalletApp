...
export const WalletAuthProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setApiBaseUrlState(normalizeApiBaseUrl(storedBaseUrl));
  ...
  const requestCode = useCallback(
    async (phoneOrEmail: string, method: 'email' | 'sms' | '') => {
      const response = await walletApi.requestCode(apiBaseUrl, { phoneOrEmail, method });

      if (!response.data) {
        throw new WalletApiError(response.message || 'Verification code could not be requested.', 400, response.message);
      }
  ...
  const verifyCode = useCallback(
    async (verificationToken: string, code: string) => {
      ...
      if (!response.data) {
        throw new WalletApiError(response.message || 'Verification failed.', 400, response.message);
      }
  ...
  const logout = useCallback(async () => {
    if (accessToken) {
      try {
        await walletApi.logout(apiBaseUrl, accessToken);
      } catch {
        // Client-side token removal still completes logout for this stateless Wallet API.
      }
    }
  ...
  const authFetch = useCallback(
    async <T,>(callback: (token: string) => Promise<T>): Promise<T> => {
      if (!accessToken) {
        throw new WalletApiError('Please sign in again.', 401);
      }
